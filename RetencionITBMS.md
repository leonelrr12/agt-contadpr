...
- [x] **EXTENSIÓN (06-09, decisión "no crear el cliente antes"): retención tolerante a clientes SIN perfil** — el tope usa el % del perfil si es agente vigente, o el 50% estándar si no lo está; la evidencia del cobro (retención explícita en la columna o **cierre exacto del neto**: efectivo = saldo − 50%×ITBMS) aplica la retención y **marca al cliente como agente automáticamente** (pct derivado de ret/ITBMS, vigencia desde la fecha de la factura). El preview avisa (`retencionAuto`, `autoMarcarCliente`, `clientesAutoMarcados`) y el confirm/resultado lo reportan. Aplica igual en PATCH /pay (`autoMarcadoAgente`). La ficha queda editable para corregir.
...
- [ ] F4 (restante): compensación automática del crédito al declarar (flujo PAGO_ITBMS / Form. 430 R52) — requiere el módulo de declaraciones. **Decisión contable APLAZADA (18-09):** el análisis, las opciones y las preguntas para el contador quedaron en `Compensacion del credito por ITBMS Retenido.md`.
